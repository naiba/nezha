...
func (ns *NotificationServerBundle) replaceParamsInString(str string, message string, mod func(string) string) string {
	...
	if ns.Server != nil {
		...
		str = strings.ReplaceAll(str, "#SERVER.SWAPTOTAL#", mod(fmt.Sprintf("%d", ns.Server.State.SwapTotal)))
		str = strings.ReplaceAll(str, "#SERVER.DISKTOTAL#", mod(fmt.Sprintf("%d", ns.Server.State.DiskTotal)))
