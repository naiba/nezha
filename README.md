...
首先在 release 下载对应的二进制解压后放置到 `/root/nezha-agent`，然后 `chmod +x /root/nezha-agent` 赋予执行权限，然后创建 `/etc/init.d/nezha`：
...
start_service() {
	procd_open_instance
	procd_set_param command /root/nezha-agent -s 面板域名:端口 -p 唯一密钥 -d
...
赋予执行权限 `chmod +x /etc/init.d/nezha` 然后启动服务 `/etc/init.d/nezha enable && /etc/init.d/nezha start`
